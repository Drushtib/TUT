...
const PostDetails = ({ initialData }) => {
  ...
  const generateStructuredData = () => {
    ...
    const structuredData = {
      ...
      "mainEntityOfPage": {
        ...
        "@id": `https://theunicorntimes.com/post/${resolvedPost?.slug?.current || resolvedPost?.slug || ""}`
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && resolvedPost) {
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    console.log('SSR - Slug type:', typeof slug);

    console.log('SSR - Slug length:', slug?.length);
    ...
    // Debug: First, check if any posts exist at all

    try {

      const allPosts = await client.fetch('*[_type == "post"][0...5]{title, "slug": slug.current}');

      console.log('SSR - Sample posts from database:', allPosts);

    } catch (error) {

      console.log('SSR - Failed to fetch sample posts:', error.message);

    }

    

    // Approach 1: Simplified query matching blogs.js structure
    ...
    try {
      ...
      console.log('SSR - Simple query:', simpleQuery.trim());
    ...
    // Approach 2: Query without escaping if first fails
    ...
    if (!initialData) {
      ...
      try {
        ...
        console.log('SSR - Fallback query:', fallbackQuery.trim());
        ...
        console.log('SSR - Fallback query failed:', error.message);
    ...
    // Approach 3: Match query with wildcard if both fail
    ...
    if (!initialData) {
      ...
      try {
        ...
        console.log('SSR - Match query:', matchQuery.trim());
